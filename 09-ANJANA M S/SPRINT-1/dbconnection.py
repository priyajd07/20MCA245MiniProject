...
def iud(querry,value):
    con=pymysql.connect(host="localhost",user="root",password="root",port=3307,db="online_exam")
    cmd=con.cursor()
...
def androidselectall(q,val):
    con=pymysql.connect(host='localhost',port=3307,user='root',passwd='',db='online_exam')
    cmd=con.cursor()
    cmd.execute(q,val)
    s=cmd.fetchall()
